...
(function() {
  ...
  if (!cookie_country && !cookie_lang) {localize()}
  ...
  function submit_localizationForm(country, lang) {
    ...
    input_return.value = create_langUrl(get_lang(), lang, window.location.href).replace("https://sportokrepselis.myshopify.com", "");
  ...
  function get_lang() {
    if (window.location.href.includes('.com/de')) {return "de"}
    else if (window.location.href.includes('.com/lt')) {return "lt"}
    else if (!window.location.href.includes('.com/de') || !window.location.href.includes('.com/lt')) {return "en"}
  }
  function create_langUrl(lang_current, lang_target, url){
    ...
    if (lang_current == 'en' && langTarget_new == '') { //if EN
      return url
  ...
  langOptions_all.forEach(x => x.addEventListener('click', (e) => {
    let lang_target = e.currentTarget.dataset.lang
    set_cookie('user_lang', lang_target, 14)
    window.location.href = create_langUrl(get_lang() , lang_target, window.location.href)

  }))
  countryOptions_all.forEach(x => x.addEventListener('click', (e) => {
