...
function filter_items(word) {
  ...
  products_All.forEach((product) => {
    if (word == "all" || product.firstElementChild.textContent == word) {
      product.style.display = "flex";
      count++;
    } else if (
      word == "sale" &&
      !product.lastElementChild.classList.contains("w-condition-invisible")
    ) {
      product.style.display = "flex";
      count++;
    } else {
...
function toggle_activeFilter(filterTag) {
  //separate funtion because of filtering on load with URL
  let word_activeBtn;
  filters_ALL.forEach((y) => {
    if (y.dataset.filter == filterTag) {
      y.classList.add("active");
      word_activeBtn = y.lastElementChild.textContent;
    } else {
...
function handle_filterText(filterTag, word_activeBtn) {
  ...
  if (filterTag == "all") {
    ...
    icon_current.src = document.querySelector(
      `img[data-filter_img="${filterTag}"`
    ).src;
...
function scroll_top() {
  if (isDesktop() || current_Display() == "tablet") {
    window.scrollTo({ top: 160, behavior: "smooth", duration: 2000 });
  } else {
    setTimeout(() => {
